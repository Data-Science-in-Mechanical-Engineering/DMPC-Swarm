...
class Plotter():
    ...
    def plot_batch_sim_results(self, plot_type, path=None, simulation_results=None, title_addon=""):
        ...
        if plot_type == 'av_transition_time':
            ...
            #    err = 1.96 * np.nanstd(transition_times[key])
            #    y = np.nanmean(transition_times[key])
            #    x = key
        ...
        if plot_type == 'dist_to_target_over_time':
            ...
            for k in range(len(simulation_results)):
                ...
                # take any __timestamp since they are identical (same simulation_frequency)
    ...
    def plot_batch_sim_results_comparison(self, plot_type, paths=None, simulation_results=None, title_addon="",
                                          labels=None):
        """ plots the batch results of the simulation run.
        ...
        Parameters:
            plot_type: string
                specify the type of plot
            paths: list(string)
                paths to the parent folder of the simulation run. Use this, if every simulation run is stored in an individual file
            simulation_results: list of simulation_result Objects
                simulation results from a simulation run stored as list
            title_addon: string
                addon to the title of the plots
        ...
        for path in paths:
            if simulation_results is None and path is None:  # to keep functionality for previously stored simulation results
                ...
                ARGS_array_list.append(ARGS_array_list)
        ...
        if plot_type == 'av_transition_time':
            ...
            for i in range(len(simulation_results_list)):

                # fill the transition_times dict
                for result in simulation_results_list[i]:

                    result.transition_times = np.array([np.NaN if result.transition_times[i] < 1 else
                                                        result.transition_times[i]
                                                        for i in range(len(result.transition_times))])
                    if f"{labels[i]} {result.num_drones}" in transition_times.keys():
                        key_temp = f"{labels[i]} {result.num_drones}"
                        transition_times[key_temp] = np.hstack((transition_times[key_temp],
                                                                         result.transition_times))
                    else:
                        transition_times[f"{labels[i]} {result.num_drones}"] = result.transition_times
        ...
        if plot_type == 'av_calculation_time':
            calculation_times = {}  # dict containing transition times sorted by the number of drones

            for i in range(len(simulation_results_list)):

                # fill the transition_times dict
                for result in simulation_results_list[i]:
                    if f"{labels[i]} {result.num_drones}" in calculation_times.keys():
                        key_temp = f"{labels[i]} {result.num_drones}"
                        calculation_times[key_temp] += result.calc_times
                    else:
                        calculation_times[f"{labels[i]} {result.num_drones}"] = result.calc_times
            ...
            data = [calculation_times[key] for key in calculation_times.keys()]
            #data = [dat[~np.isnan(dat)] for dat in data]
            x = [key for key in calculation_times.keys()]
        ...
        if plot_type == 'av_num_constraints':
            num_constraints = {}  # dict containing transition times sorted by the number of drones

            for i in range(len(simulation_results_list)):

                # fill the transition_times dict
                for result in simulation_results_list[i]:
                    if f"{labels[i]} {result.num_drones}" in num_constraints.keys():
                        key_temp = f"{labels[i]} {result.num_drones}"
                        num_constraints[key_temp] += result.num_constraints
                    else:
                        num_constraints[f"{labels[i]} {result.num_drones}"] = result.num_constraints
            ...
            data = [num_constraints[key] for key in num_constraints.keys()]
            #data = [dat[~np.isnan(dat)] for dat in data]
            x = [key for key in num_constraints.keys()]
