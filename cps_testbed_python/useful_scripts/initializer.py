...
class Initializer():
    ...
    def initialize(self, init_type, dist_to_wall, num_points, min_dist, scaling_factor=1, num_targets = None):
        ...
        if init_type == 'random':
            ...
            # subcubes = self._divide_cube_in_subcubes(spawn_cube)

            # make sure enough points are generated
            # num_points = max(math.ceil(self.__num_drones / len(subcubes)) * len(subcubes), num_points)

            try:
                # print('Calculating Initial Positions for ' + str(num_points) + ' Drones.')
                drone_locations = self.generate_points__min_distance(num_points=num_points + num_targets,
                                                                     shape=spawn_cube, min_dist=min_dist,
                                                                     max_iter=20000, scaling_factor=scaling_factor)
            except ValueError:

                print('Cannot fit enough positions with guaranteed margins, using minmax algorithm instead.')
                drone_locations = hycusampling.maximin_reconstruction(num_points + num_targets, 3)
                drone_locations = self._scale_points_to_testbed(drone_locations, spawn_cube)



            INIT_XYZS = drone_locations[:num_points]
            INIT_TARGETS = drone_locations[num_points:]
            ...
            try:
                ...
                print('Error, using minmax algorithm instead.')
                drone_locations = hycusampling.maximin_reconstruction(num_points, 3)
                drone_locations = self._scale_points_to_testbed(drone_locations, spawn_cube)

            try:
                # print('Calculating Target Positions for ' + str(num_points) + ' Drones.')
                target_locations = self.generate_points__min_distance(num_targets, spawn_cube, min_dist=min_dist,
                                                                      max_iter=2000, scaling_factor=scaling_factor)

            except ValueError:
                # print('Error, using minmax algorithm instead.')
                target_locations = hycusampling.maximin_reconstruction(num_targets, 3)
