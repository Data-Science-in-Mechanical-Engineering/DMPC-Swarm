...
class Network:
    ...
    def __init__(self, agents=None, message_loss=0.1, ack_message_loss=0, slot_group_ack_id=100000, rounds_lost=None):
        ...
        self.__ack_message_loss = ack_message_loss

        self.__slot_group_ack = SlotGroup(slot_group_ack_id, False, 0)
    ...
    def add_agent(self, agent):
        ...
        # update slot group
        self.__slot_group_ack = SlotGroup(100000, False, len(self.__agents))
    ...
    def step(self):
        ...
        for slot_group in self.__slot_groups:
            for agent_scheduled in slot_group.schedule:
                ...
                # else:
                me = agent_scheduled.get_message(slot_group.id)
                if me is not None:
        ...
        messages_lost = {self.__agents[i].ID: messages_lost[i] for i in range(len(messages_lost))}

        messages_ack = [Message(ID=self.__agents[i].ID,
                                slot_group_id=self.__slot_group_ack.id,
                                content=AcknowledgementMessageContent({}))
                        for i in range(0, len(self.__agents))]
            ...
            for agent in self.__agents:
                if random.random() < self.__message_loss and (self.__round in self.__rounds_lost):
                    ...
                    for m in messages_ack:
                        ...
                        m.content.messages_received[(agent.ID, message.ID, message.slot_group_id)] = False
                else:
                    agent.send_message(message)
                    for m in messages_ack:
                        # when the message was received, acknowledgement might be true or false
                        m.content.messages_received[(agent.ID, message.ID, message.slot_group_id)] = random.random() > self.__ack_message_loss
            i += 1

        # send acknowledgement to agents
        i = 0
        for agent in self.__agents:
            agent.send_message(messages_ack[i])  # the acknowledgement flag is then send to the agent, because it is an aggregated message it is not many to all.
            i += 1
...
@dataclass
class AcknowledgementMessageContent:
