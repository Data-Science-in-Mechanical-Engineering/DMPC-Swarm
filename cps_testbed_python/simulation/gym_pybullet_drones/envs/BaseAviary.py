...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.GUI:
            ...
            self.CLIENT = bc.BulletClient(
                connection_mode=p.GUI, options="--opengl2")  # self.CLIENT.connect(p.GUI, options="--opengl2")
            for i in [self.CLIENT.COV_ENABLE_RGB_BUFFER_PREVIEW, self.CLIENT.COV_ENABLE_DEPTH_BUFFER_PREVIEW,
                      self.CLIENT.COV_ENABLE_SEGMENTATION_MARK_PREVIEW]:
                self.CLIENT.configureDebugVisualizer(i, 0, physicsClientId=self.CLIENT._client)
            ...
            ret = self.CLIENT.getDebugVisualizerCamera(physicsClientId=self.CLIENT._client)
            ...
            # if platform == "linux": self.CLIENT.setAdditionalSearchPath(pybullet_data.getDataPath()); plugin = self.CLIENT.loadPlugin(egl.get_filename(), "_eglRendererPlugin"); print("plugin=", plugin)
            if self.RECORD:
                ...
                self.CAM_VIEW = self.CLIENT.computeViewMatrixFromYawPitchRoll(distance=3,
                                                                              yaw=-30,
                                                                              pitch=-30,
                                                                              roll=0,
                                                                              cameraTargetPosition=[0, 0, 0],
                ...
                self.CAM_PRO = self.CLIENT.computeProjectionMatrixFOV(fov=60.0,
    ...
    def reset(self):
        ...
        """
        self.CLIENT.resetSimulation(physicsClientId=self.CLIENT._client)
        #### Housekeeping ##########################################
        self._housekeeping()
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
    ...
    def step(self,
        ...
        if self.RECORD and not self.GUI and self.step_counter % self.CAPTURE_FREQ == 0:
            [w, h, rgb, dep, seg] = self.CLIENT.getCameraImage(width=self.VID_WIDTH,
                                                               height=self.VID_HEIGHT,
                                                               shadow=1,
                                                               viewMatrix=self.CAM_VIEW,
                                                               projectionMatrix=self.CAM_PRO,
                                                               renderer=self.CLIENT.ER_TINY_RENDERER,
                                                               flags=self.CLIENT.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX,
                                                               physicsClientId=self.CLIENT._client
                                                               )
        ...
        for _ in range(self.AGGR_PHY_STEPS):
            ...
            if self.PHYSICS != Physics.DYN:
                self.CLIENT.stepSimulation(physicsClientId=self.CLIENT._client)
    ...
    def close(self):
        ...
        if self.RECORD and self.GUI:
            self.CLIENT.stopStateLogging(self.VIDEO_ID, physicsClientId=self.CLIENT._client)
        p.disconnect(physicsClientId=self.CLIENT._client)
    ...
    def _housekeeping(self):
        ...
        self.CLIENT.setGravity(0, 0, -self.G, physicsClientId=self.CLIENT._client)
        self.CLIENT.setRealTimeSimulation(0, physicsClientId=self.CLIENT._client)
        self.CLIENT.setTimeStep(self.TIMESTEP, physicsClientId=self.CLIENT._client)
        self.CLIENT.setAdditionalSearchPath(pybullet_data.getDataPath(), physicsClientId=self.CLIENT._client)
        #### Load ground plane, drone and obstacles models #########
        self.PLANE_ID = self.CLIENT.loadURDF("plane.urdf", physicsClientId=self.CLIENT._client)
        self.DRONE_IDS = np.array(
            [self.CLIENT.loadURDF(os.path.dirname(os.path.abspath(__file__)) + "/../assets/" + self.URDF,
                                  self.INIT_XYZS[i, :],
                                  self.CLIENT.getQuaternionFromEuler(self.INIT_RPYS[i, :]),
        ...
        for i in range(self.NUM_DRONES):
            ...
            # self.CLIENT.setCollisionFilterPair(bodyUniqueIdA=self.PLANE_ID, bodyUniqueIdB=self.DRONE_IDS[i], linkIndexA=-1, linkIndexB=-1, enableCollision=0, physicsClientId=self.CLIENT._client)
        if self.OBSTACLES:
            self._addObstacles()
    ...
    def _updateAndStoreKinematicInformation(self):
        ...
        """
        #print("asdasdfadfasdfTESadsfgSRG")
        #print(self.CLIENT._client)
        #if self.CLIENT._client == 1:
        #    print("Hello There")
        for i in range(self.NUM_DRONES):
            self.pos[i], self.quat[i] = self.CLIENT.getBasePositionAndOrientation(self.DRONE_IDS[i])
            self.rpy[i] = self.CLIENT.getEulerFromQuaternion(self.quat[i])
            self.vel[i], self.ang_v[i] = self.CLIENT.getBaseVelocity(self.DRONE_IDS[i],
                                                                     physicsClientId=self.CLIENT._client)
    ...
    def _startVideoRecording(self):
        ...
        """
        if self.RECORD and self.GUI:
            self.VIDEO_ID = self.CLIENT.startStateLogging(loggingType=self.CLIENT.STATE_LOGGING_VIDEO_MP4,
    ...
    def _getDroneImages(self,
        ...
        if self.IMG_RES is None:
            ...
            exit()
        rot_mat = np.array(self.CLIENT.getMatrixFromQuaternion(self.quat[nth_drone, :])).reshape(3, 3)
        ...
        DRONE_CAM_PRO = self.CLIENT.computeProjectionMatrixFOV(fov=60.0,
                                                               aspect=1.0,
                                                               nearVal=self.L,
                                                               farVal=1000.0
                                                               )
        SEG_FLAG = self.CLIENT.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX if segmentation else self.CLIENT.ER_NO_SEGMENTATION_MASK
        [w, h, rgb, dep, seg] = self.CLIENT.getCameraImage(width=self.IMG_RES[0],
    ...
    def _groundEffect(self,
        ...
        link_states = np.array(self.CLIENT.getLinkStates(self.DRONE_IDS[nth_drone],
    ...
    def _drag(self,
        ...
        base_rot = np.array(self.CLIENT.getMatrixFromQuaternion(self.quat[nth_drone, :])).reshape(3, 3)
    ...
    def _dynamics(self,
        ...
        self.CLIENT.resetBasePositionAndOrientation(self.DRONE_IDS[nth_drone],
                                                    pos,
                                                    self.CLIENT.getQuaternionFromEuler(rpy),
                                                    physicsClientId=self.CLIENT._client
                                                    )
        #### Note: the base's velocity only stored and not used ####
        self.CLIENT.resetBaseVelocity(self.DRONE_IDS[nth_drone],
                                      vel,
    ...
    def _addObstacles(self):
        ...
        """
        self.CLIENT.loadURDF("samurai.urdf",
                             physicsClientId=self.CLIENT._client
                             )
        self.CLIENT.loadURDF("duck_vhacd.urdf",
                             [-.5, -.5, .05],
                             self.CLIENT.getQuaternionFromEuler([0, 0, 0]),
                             physicsClientId=self.CLIENT._client
                             )
        self.CLIENT.loadURDF("cube_no_rotation.urdf",
                             [-.5, -2.5, .5],
                             self.CLIENT.getQuaternionFromEuler([0, 0, 0]),
                             physicsClientId=self.CLIENT._client
                             )
        self.CLIENT.loadURDF("sphere2.urdf",
                             [0, 2, .5],
                             self.CLIENT.getQuaternionFromEuler([0, 0, 0]),
                             physicsClientId=self.CLIENT._client
                             )
