"""
Contains code for data interpolation
"""
...
class TrajectoryCoefficients:
    ...
    ----------
        ...
        valid: bool
            true if coefficients represent a valid data or not (because for example data is not calculated
...
class Interpolation(ABC):
    """this abstract class represents the Interpolation for the data generation"""
    ...
    def get_trajectory_vector_matrix(self, x_vector, derivative_order=0):
        """calculates the matrix that allows to calculate the data by multiplying it with the optimization variables
...
class PiecewiseBernsteinPolynomial(Interpolation):
    ...
    def get_trajectory_vector_matrix(self, x_vector, derivative_order=0):
        """calculates the matrix that allows to calculate the data by multiplying it with polynomial_coefficients

        when putting all polynomial coefficients into one big vector, one can get a big vector of the points of the
        data at timepoints stored in x_vector by multipling the vecotr of polynomial coefficients with the matrix
        this method returns
    ...
    def equality_constraint(self):
        ...
        such that the resulting data is continuous.
        Returns
        -------
            A: np.array, shape(num_equality_constraints, num_optimization_variables)
            b: np.array, shape(num_equality_constraints,)
    ...
    def build_continuity_constraint_matrix(self, derivative_order=0):
        ...
        This matrix can be used to build start and final equality constraints on the data and
