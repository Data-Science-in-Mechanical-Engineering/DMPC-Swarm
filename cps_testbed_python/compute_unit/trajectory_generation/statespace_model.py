...
from compute_unit.trajectory_generation.interpolation import Interpolation, TrajectoryCoefficients
...
class StateSpace(Interpolation):
    ...
    def __init__(self, breakpoints, dimension_trajectory, num_states, sampling_time, model_order):
        ...
        ----------
            breakpoints: np.array, shape(k_start+1,)
                breakpoints of the interpolation. The data has k_start optimization variables
            dimension_trajectory: int
                dimension of data (typically 3 or 2)
            num_states: int
                number of states
            num_inputs: int
                number of inputs
    ...
    def get_input_trajectory_vector_matrix(self, x_vector, derivative_order=0):
        """calculates the matrix that allows to calculate the data from the input by multiplying it with the
        optimization variables

        when putting all polynomial coefficients into one big vector, one can get a big vector of the points of the
        data at timepoints stored in x_vector by multiplying the vecotr of polynomial coefficients with the matrix
        this method returns

        Parameters
        ----------
            x_vector: np.array, shape(l,)
                time points the data should be calculated at
        ...
        -------
            matrix: np.array, shape(dimension_trajectory*len(x_vector), num_optimization_variables)
                ...
                flattened with C-style, we can get the data by [y(x_vector[0], ...)] =
        ...
        for i in range(len(x_vector)):
            full_segments = int(x_vector[i] / self.__sampling_time)
            dT = x_vector[i] - full_segments * self.__sampling_time

            for j in range(full_segments):
                variable_pos = min(j, len(self.__breakpoints)-2)  # if system is at the end of input data, simply extrapolate with last input
                matrix[i * self.dimension:(i + 1) * self.dimension,
                variable_pos * self.dimension:(variable_pos + 1) * self.dimension] += psi @ self.discrete_state_matrix(
                    dT) @ np.linalg.matrix_power(
                    A_0, full_segments - j - 1) @ B_0  # +=, because if the data is longer than breakpoints, this is needed

            if dT > 0:
                matrix[i * self.dimension:(i + 1) * self.dimension,
                min(full_segments, len(self.__breakpoints)-2)  * self.dimension:(min(full_segments, len(self.__breakpoints)-2) + 1) * self.dimension] += psi @ self.discrete_input_matrix(dT)
    ...
    def get_state_trajectory_vector_matrix(self, x_vector, derivative_order=0):
        ...
        for i in range(len(x_vector)):
            ...
            dT = max(dT - int(dT / self.__sampling_time) * self.__sampling_time, 0)
        ...
        """
                ...
                ----------
                    ...
                    integration_start: float
                        __timestamp, at which the interpolation should start
...
class TripleIntegrator(StateSpace):
    ...
    def __init__(self, breakpoints, dimension_trajectory, num_states, sampling_time):
        ...
        ----------
            breakpoints: np.array, shape(k_start+1,)
                breakpoints of the integration. The data has k_start optimization variables
            dimension_trajectory: int
                dimension of data (typically 3 or 2)
            num_states: int
                number of states
